...
class Note {
  ...
  Note copyWith({String? text, bool? pinned, DateTime? updatedAt, int? colorHex, bool keepNullColor = false}) => Note(
        id: id,
        text: text ?? this.text,
        createdAt: createdAt,
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              'Нажми «Новая», чтобы создать первую запись.',
          ...
          Row(
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  final now = DateTime.now();
                  final dateStr = '${_pad(now.day)}.${_pad(now.month)}.${now.year} ${_pad(now.hour)}:${_pad(now.minute)}';
                  final selection = _ctrl.selection;
                  final text = _ctrl.text;
                  final inserted = text.replaceRange(
                  ...
                  _ctrl.selection = TextSelection.collapsed(
                    offset: (selection.start >= 0 ? selection.start : text.length) + dateStr.length);
                },
          ...
          Align(
            ...
            child: Wrap(
              ...
              children: [
                ...
                for (final c in _palette(context))
                  _ColorDot(
                    color: c,
...
String _formatDate(DateTime dt) {
  ...
  if (isToday) return '${_pad(dt.hour)}:${_pad(dt.minute)}';
